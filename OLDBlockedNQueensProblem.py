...
def checkForwardAttempt(row, column, chessBoard, n):
    ...
    for i in range(column + 1, n):
        if checkForCurrentCellFeasible(chessBoard, row, i) and chessBoard[row][i] != 'X':
            chessBoard[row][i] = 'x'
    ...
    for index in range(1, n):
        index_row = row - index
        index_col = column + index
        if checkBounds(index_row, index_col, n) and checkForCurrentCellFeasible(chessBoard, index_row, index_col) and chessBoard[index_row][index_col] != 'X':
                ...
                for k in range(n):
                    ...
                    if conditionForBacktracking:  # and chessBoard[i][columnToInsertValue] != 'X' and chessBoard[i][columnToInsertValue] != 'Q':
                        ...
                        while indexTillLastRow < n:
                            if checkBounds(indexTillLastRow, backupColumnForBacktracking, n) and \
                                    chessBoard[indexTillLastRow][backupColumnForBacktracking] != 'X' and \
                                    chessBoard[indexTillLastRow][backupColumnForBacktracking] != 'x':
...
def checkAttemptWithMAC(chessBoard, row, column, n):
    ...
    for index in range(row - 1, row + 2):
        if checkBounds(index, columnToScan, n) and checkForCurrentCellFeasible(chessBoard, index, columnToScan):
